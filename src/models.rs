...
impl TryFrom<u8> for NetworkStatus {
    ...
    fn try_from(original: u8) -> Result<Self, Self::Error> {
        match original {
            ...
            10 => Ok(NetworkStatus::NetworkStopping),
            11 => Ok(NetworkStatus::NetworkStopped),
            12 => Ok(NetworkStatus::NetworkBlocked),
            13 => Ok(NetworkStatus::NetworkUnavailable),
            14 => Ok(NetworkStatus::NetworkError),
            15 => Ok(NetworkStatus::Pausing),
            16 => Ok(NetworkStatus::Paused),
            17 => Ok(NetworkStatus::Unpausing),
            18 => Ok(NetworkStatus::ShuttingDown),
            19 => Ok(NetworkStatus::GracefullyShuttingDown),
            20 => Ok(NetworkStatus::Shutdown),
            21 => Ok(NetworkStatus::GracefullyShutdown),
            22 => Ok(NetworkStatus::Restarting),
            23 => Ok(NetworkStatus::GenError),
            _ => Err(())
        }
    }
}
